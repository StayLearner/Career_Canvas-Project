  ...
  <Dialog open={open} onOpenChange={setOpen}>
    ...
    >
      ...
      <form onSubmit={submitHandler} id="update-profile-form" className="flex flex-col max-h-[calc(90vh-96px)]">
        ...
        {/* Sticky Footer */}
      <DialogFooter className="sticky bottom-0 bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-xl border-t border-slate-100 dark:border-white/10 px-6 py-4 flex flex-row justify-end gap-3">
          <button
            type="button"
            onClick={() => setOpen(false)}
            className="h-10 px-5 rounded-xl text-sm text-slate-500 hover:text-slate-800 dark:hover:text-white transition"
          >
            Cancel
          </button>

          {loading ? (
            <Button disabled className="h-10 rounded-xl">
            ...
            <Button
              ...
              className="h-10 rounded-xl bg-cyan-500 hover:bg-cyan-600 text-white font-semibold px-6"
            >
              Update Profile
            </Button>
          )}
