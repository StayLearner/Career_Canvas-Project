                    ...
                    <div className='flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-5 pb-6 border-b border-slate-100 dark:border-white/5'>
                        <div className='flex flex-col sm:flex-row sm:items-center gap-4 text-left'>
                            ...
                            <div className="space-y-1">
                                <h1 className='font-bold text-xl sm:text-2xl text-slate-900 dark:text-white'>{user?.fullname}</h1>
                                <p className='text-sm sm:text-base text-slate-600 dark:text-slate-400 break-words'>{user?.profile?.bio || "No bio added yet"}</p>
                            </div>
                        </div>
                        <Button 
                        ...
                        <div className='flex flex-wrap items-center gap-2 mt-3'>
                            {
                                user?.profile?.skills?.length !== 0 ? (
                                    user?.profile?.skills?.map((item, index) => (
                                        <Badge 
                                            key={index}
                                            className="bg-cyan-500/10 text-cyan-650 dark:text-cyan-400 border border-cyan-500/20 text-xs font-semibold px-2.5 py-1 rounded-lg hover:bg-cyan-500/20 shadow-none transition"
                                        >
                                            {item}
                                        </Badge>
                                    ))
